...
namespace Riptide.Transports.Eos {
    ...
    public class EosClient : EosPeer, IClient {
        ...
        private EosConnection udpConnection;

        /// <inheritdoc/>
        public EosClient(SocketMode mode = SocketMode.Both, int socketBufferSize = DefaultSocketBufferSize) : base(mode, socketBufferSize) { }
        ...
        public bool Connect(string hostAddress, out Connection connection, out string connectError) {
            ...
            connection = udpConnection = new EosConnection(new IPEndPoint(mode == SocketMode.IPv4Only ? ip : ip.MapToIPv6(), port), this);
